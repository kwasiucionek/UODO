...
def extract_tags_with_llm(query: str, available_tags: list[str]) -> list[str]:
    ...
    Używa call_llm_json z llm.py — jednolite wywołanie przez jedną ścieżkę.
    """
...
# Maksymalna liczba decyzji jaką może zwrócić jeden tag —
# tagi zwracające więcej są zbyt ogólne i zaśmiecają wyniki.
...
def hybrid_search(
    ...
    query        — oryginalne zapytanie użytkownika (do ekstrakcji tagów i fraz)
    search_query — wzbogacone zapytanie z LLM (do semantic search); jeśli None używa query
    ...
    """
    sem_query    = search_query or query   # semantic search używa enriched query
    matched_tags = get_matched_tags(query) # tagi zawsze z oryginalnego zapytania
    ...
    # 1b. Frazy 2-wyrazowe z oryginalnego zapytania → scroll po tagu BEZ LLM
    # Używamy TYLKO par sąsiednich słów — pojedyncze słowa ("dane", "zgoda")
    # są zbyt ogólne i zwracają setki niezwiązanych decyzji.
    # "dane genetyczne" → para dwuwyrazowa → dokładnie 26 decyzji z tagiem.
    words = [
        w.lower() for w in re.split(r"\W+", query)
        if w.lower() not in QUERY_STOPWORDS and len(w) > 2
    ]
    two_word_phrases: list[str] = list(dict.fromkeys(
    ...
    for tag in direct_hits:
        results = keyword_exact_search(tag, {**filters_base, "doc_types": ["uodo_decision"]})
        # Pomiń tagi zwracające za dużo wyników — są zbyt ogólne
        if len(results) <= _MAX_RESULTS_PER_TAG:
            for d in results:
                _add(decisions, d)

    # 1c. Tagi LLM — tylko gdy 1b nic nie znalazło (fraza nie jest tagiem w bazie)
    if not decisions and matched_tags:
        for tag in matched_tags:
            results = keyword_exact_search(tag, {**filters_base, "doc_types": ["uodo_decision"]})
            # Też filtruj zbyt ogólne tagi LLM
            if len(results) <= _MAX_RESULTS_PER_TAG:
                for d in results:
                    _add(decisions, d)

    # 1d. Semantic search — ostatni fallback gdy tagi nic nie dały
    # Używa sem_query (enriched) dla lepszego dopasowania semantycznego
    if len(decisions) < 5:
