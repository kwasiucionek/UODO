...
def main() -> None:
    ...
    if effective_query and (
        ...
        # Wyczyść poprzednią odpowiedź gdy zapytanie się zmieniło
        if st.session_state.get("last_answer_query") != effective_query:
            st.session_state.pop("last_answer", None)
            st.session_state.pop("last_answer_query", None)
        ...
        # effective_query (oryginalne) trafia do hybrid_search jako podstawa ekstrakcji tagów.
        ...
        if use_llm:
            ...
            # Odtwórz poprzednią odpowiedź jeśli zapytanie się nie zmieniło —
            # bez tego odpowiedź znika przy każdym rerenderze Streamlit.
            if st.session_state.get("last_answer_query") == effective_query:
                full_answer = st.session_state.get("last_answer", "")
                if full_answer:
            ...
            if not full_answer:
                try:
                    ...
                    # Zapisz do session_state żeby przeżyło rerender
                    st.session_state["last_answer"]       = full_answer
                    st.session_state["last_answer_query"] = effective_query
